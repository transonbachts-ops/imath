...
export default async function Home() {
  ...
  try {
    ...
    // Fetch top 3 students (Clean up old streaks in real-time)
    const vnTimeNow = new Date(new Date().getTime() + 7 * 60 * 60 * 1000);
    const yesterdayDate = new Date(vnTimeNow.getTime() - 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  ...
  const dashboardLink = user ? (user.role === 'parent' ? '/parent/dashboard' : (user.role === 'admin' ? '/admin' : '/dashboard')) : '/login';
    ...
    <div style={{fontFamily: 'system-ui, sans-serif', color: '#333'}}>
      ...
      <nav style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '25px 60px', position: 'absolute', top: 0, left: 0, right: 0, zIndex: 100, color: '#fff'}}>
        ...
        <div style={{display: 'flex', gap: 40, alignItems: 'center'}}>
          ...
          <Link href="/parent" style={{color: '#fff', textDecoration: 'none', fontWeight: 700, fontSize: 14, opacity: 0.9}}>Phụ huynh</Link>
          <Link href="/leaderboard" style={{color: '#fff', textDecoration: 'none', fontWeight: 700, fontSize: 14, opacity: 0.9}}>Bảng xếp hạng</Link>
          
          {user ? (
            <Link href={dashboardLink} style={{color: 'var(--secondary)', textDecoration: 'none', fontWeight: 800, fontSize: 14}}>
               Chào {user.full_name} 👋
            </Link>
          ) : (
          ...
          <Link href={user ? dashboardLink : "/register"} style={{background: '#fff', color: 'var(--primary)', padding: '12px 30px', borderRadius: 14, textDecoration: 'none', fontWeight: 800, fontSize: 14, boxShadow: '0 10px 25px rgba(0,0,0,0.1)'}}>
                 ...
                 <div key={c.id} className="glass-panel floating" style={{overflow: 'hidden', padding: 0, border: '1px solid var(--glass-border)'}}>
                    ...
                    <div style={{padding: 35}}>
                       ...
                       <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border-muted)', paddingTop: 25}}>
                          <Link href={user ? `/course/${c.id}` : "/login"} className="btn btn-primary" style={{padding: '12px 30px', fontSize: 14, borderRadius: 12}}>Vào Lớp →</Link>
      ...
      {/* LEADERBOARD PODIUM */}
      <section style={{padding: '50px 50px 0 50px', background: 'transparent'}}>
         <div style={{maxWidth: 1100, margin: '0 auto'}}>
            ...
            {topStudents.length > 0 ? (
               <div style={{display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: 20, minHeight: 300, paddingBottom: 0}}>
                  {/* #2 */}
                  {topStudents[1] && (
                     <div style={{width: 200, display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
                        <div style={{fontWeight: 800, fontSize: 18, color: '#333'}}>{topStudents[1].full_name}</div>
                        <div style={{fontSize: 14, color: '#e67e22', fontWeight: 900, marginBottom: 15}}>🔥 {topStudents[1].current_streak} ngày</div>
                        <div style={{width: '100%', height: 140, background: 'linear-gradient(to top, #bdc3c7, #ecf0f1)', borderRadius: '12px 12px 0 0', display: 'flex', justifyContent: 'center', paddingTop: 15, fontSize: 30, fontWeight: 900, color: '#7f8c8d', boxShadow: '0 -5px 15px rgba(0,0,0,0.05)'}}>2</div>
                     </div>
                  )}
                  {/* #1 */}
                  {topStudents[0] && (
                     <div style={{width: 220, display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
                        <div style={{fontSize: 35, marginBottom: 5, animation: 'pulse 2s infinite'}}>👑</div>
                        <div style={{fontWeight: 900, fontSize: 20, color: '#333'}}>{topStudents[0].full_name}</div>
                        <div style={{fontSize: 15, color: '#e67e22', fontWeight: 900, marginBottom: 15}}>🔥 {topStudents[0].current_streak} ngày</div>
                        <div style={{width: '100%', height: 180, background: 'linear-gradient(to top, #f39c12, #f1c40f)', borderRadius: '12px 12px 0 0', display: 'flex', justifyContent: 'center', paddingTop: 15, fontSize: 40, fontWeight: 900, color: '#fff', boxShadow: '0 -10px 25px rgba(243,156,18,0.3)'}}>1</div>
                     </div>
                  )}
                  {/* #3 */}
                  {topStudents[2] && (
                     <div style={{width: 200, display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
                        <div style={{fontWeight: 800, fontSize: 18, color: '#333'}}>{topStudents[2].full_name}</div>
                        <div style={{fontSize: 14, color: '#e67e22', fontWeight: 900, marginBottom: 15}}>🔥 {topStudents[2].current_streak} ngày</div>
                        <div style={{width: '100%', height: 110, background: 'linear-gradient(to top, #e67e22, #f39c12)', borderRadius: '12px 12px 0 0', display: 'flex', justifyContent: 'center', paddingTop: 15, fontSize: 30, fontWeight: 900, color: '#fff', boxShadow: '0 -5px 15px rgba(211,84,0,0.2)'}}>3</div>
                     </div>
                  )}
               </div>
            ) : (
               <div style={{textAlign: 'center', padding: 40, color: '#888'}}>Chưa có kỷ lục nào được thiết lập</div>
            )}
         </div>
      </section>
