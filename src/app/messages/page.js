...
export default function UnifiedMessengerPage() {
  ...
  const fetchMessages = async (contact) => {
    if (!contact) return;
    try {
      const senderParam = contact.senderId ? `&sender_id=${contact.senderId}` : '';
      const res = await fetch(`/api/parent/messages?student_id=${contact.studentId}&course_id=${contact.courseId}&other_user_id=${contact.contactId}${senderParam}`);
      if (res.ok) {
        const data = await res.json();
        const newMsgs = data.messages || [];
        // Chỉ update nếu số tin nhắn thay đổi (tránh re-render không cần thiết)
        setMessages(prev => {
  ...
  useEffect(() => {
    if (activeContact) {
      lastMsgCountRef.current = 0; // Reset khi đổi contact
  ...
  // Smart auto-scroll: chỉ cuộn xuống khi user đang ở gần đáy HOẶC có tin mới
  useEffect(() => {
    if (!scrollRef.current) return;
    const { scrollTop, scrollHeight, clientHeight } = scrollRef.current;
    const isNearBottom = scrollHeight - scrollTop - clientHeight < 120;
    const hasNewMessage = messages.length > lastMsgCountRef.current;
  ...
  const handleSend = async (content = '', attachment_url = null, attachment_type = null) => {
    ...
    // Fix: Nếu contactId chính là tôi (thường xảy ra với admin view khi recv=mình),
    // thì gửi đến senderId — người đã nhắn cho tôi
    const recipientId = (activeContact.contactId === user?.id && activeContact.senderId)
         ...
         <div style={{width: 380, background: '#fff', borderRadius: 24, border: '1px solid #eef2f7', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 5px 25px rgba(0,51,128,0.03)'}}>
           ...
           <div style={{flex: 1, overflowY: 'auto', padding: 15, display: 'flex', flexDirection: 'column', gap: 10}}>
             {contacts.length === 0 ? (
               ...
               contacts.map((c, i) => {
                 const isActive = activeContact?.contactId === c.contactId && activeContact?.courseId === c.courseId && activeContact?.senderId === c.senderId;
                 return (
                   <button 
                     key={i} 
                   ...
                   >
                     ...
                     <div style={{flex: 1, overflow: 'hidden'}}>
                       ...
                       <div style={{fontSize: 12, color: isActive ? '#0284c7' : '#64748b', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}}>
                         {user?.role === 'admin' ? `Học sinh: ${c.studentName}` : (c.role === 'teacher' ? `Lớp: ${c.courseTitle}` : `PH của bé: ${c.studentName}`)}
                       </div>
                     </div>
                   </button>
                 );
