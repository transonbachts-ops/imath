    ...
    <div style={{position: 'relative'}} ref={menuRef}>
      ...
      {open && (
        <div className="glass-panel" style={{
          position: 'absolute', top: 52, right: 0, width: 260, background: 'var(--card-bg)', borderRadius: 16,
          overflow: 'hidden', zIndex: 999, border: '1px solid var(--border-muted)', boxShadow: '0 15px 40px rgba(0,0,0,0.15)'
        }}>
           {/* Header */}
           ...
           <div style={{padding: '8px 0'}}>
              ...
              <Link href="/my-certificates" onClick={() => setOpen(false)} style={{display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', color: 'var(--text-primary)', textDecoration: 'none', fontSize: 14}}>
                 <span style={{fontSize: 18}}>🎓</span> Chứng chỉ của tôi
              </Link>
              ...
              <Link href="/leaderboard" onClick={() => setOpen(false)} style={{display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', color: 'var(--text-primary)', textDecoration: 'none', fontSize: 14}}>
                 <span style={{fontSize: 18}}>🏆</span> Bảng Xếp Hạng Chuyên Cần
              </Link>
              {user.role === 'student' && (
                 <Link href="/daily" onClick={() => setOpen(false)} style={{display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', color: 'var(--text-primary)', textDecoration: 'none', fontSize: 14}}>
                   <span style={{fontSize: 18}}>🔥</span> Bài Toán Hằng Ngày
                 </Link>
              )}
              {(user.role === 'admin' || user.role === 'teacher') && (
                 <Link href={user.role === 'admin' ? '/admin' : '/teacher/dashboard'} onClick={() => setOpen(false)} style={{display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', color: 'var(--text-primary)', textDecoration: 'none', fontSize: 14}}>
                   <span style={{fontSize: 18}}>⚙️</span> {user.role === 'admin' ? 'Trang Quản Trị' : 'Dashboard Giảng Viên'}
                 </Link>
              )}
              {user.role === 'student' && (
