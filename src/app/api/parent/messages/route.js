...
export async function GET(req) {
  ...
  const senderIdRequested = searchParams.get('sender_id');
  ...
  try {
    // Mark incoming messages as read (only if actually for me)
    if (otherUserId && user.role !== 'admin') {
      await pool.query(
    ...
    if (user.role === 'admin' && senderIdRequested && otherUserId) {
      // PURE MONITOR MODE: Admin sees exactly the conversation between A and B
      queryOptions = `WHERE pm.student_id = ? AND ((pm.sender_id = ? AND pm.receiver_id = ?) OR (pm.sender_id = ? AND pm.receiver_id = ?))`;
      queryParams = [studentId, senderIdRequested, otherUserId, otherUserId, senderIdRequested];
    } else if (user.role === 'admin' && otherUserId) {
      // COMPATIBILITY MODE: Admin sees ALL messages involving this person as sender/receiver
