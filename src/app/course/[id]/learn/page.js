...
import RulesClientWrapper from '@/app/components/RulesClientWrapper';
...
export default async function CourseLearnPage({ params }) {
  ...
  const isEnrolled = enrolls.length > 0;
  
  if (!isEnrolled && user.role !== 'admin' && user.role !== 'teacher') {
    redirect(`/course/${id}`);
  }

  const enroll = enrolls[0] || { created_at: new Date(), rules_accepted: 1 };
  const enrollDate = new Date(enroll.created_at).toLocaleDateString('vi-VN');
  
  // Rules check
  const showRules = !enroll.rules_accepted && user.role !== 'admin' && user.role !== 'teacher';

  // Load modules & activities (Rest of the logic)
  ...
  const nextActivity = courseActivities.find(a => !completedIds.includes(a.id));
  ...
  const getActivityIcon = (type) => {
    ...
    if (type === 'interactive_video') return '🎬';
    if (type === 'game') return '🎮';
    return '📄';
  };
  ...
  return (
    <div style={{fontFamily: "'Segoe UI', system-ui, sans-serif", background: '#f0f4f8', minHeight: '100vh', display: 'flex'}}>
      {/* MANDATORY RULES AGREEMENT */}
      <RulesClientWrapper 
        courseId={id} 
        rules={course.rules} 
        alreadyAccepted={!showRules} 
      />
            ...
            <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 20px', color: '#003380', fontSize: 14, fontWeight: 'bold', background: '#f0f4f8', borderLeft: '3px solid #003380'}}>
             ...
             <div>
                {courseModules.map((m) => {
                   ...
                   return (
                     <div key={m.id}>
                        <div style={{padding: '10px 20px 10px 40px', color: '#444', fontSize: 13, fontWeight: 600, background: '#f9fafb', borderTop: '1px solid #f0f0f0', borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                          <Link href={`/course/${id}/module/${m.id}`} style={{color: '#003380', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8}}>
                             📂 {m.title}
                          </Link>
                        </div>
                              ...
                              <div key={act.id} style={{padding: '8px 15px 8px 0', fontSize: 13, display: 'flex', gap: 8, alignItems: 'center', borderBottom: '1px solid #f9f9f9'}}>
                                ...
                                <Link 
                                   href={
                                     ...
                                     act.type === 'interactive_video' ? `/course/${id}/interactive-video/${act.id}` :
                                     act.type === 'game' ? `/course/${id}/game/${act.id}` : 
                                     act.type === 'assignment' ? `/course/${id}/assignment/${act.id}` :
                                     act.type === 'forum' ? `/course/${id}/forum/${act.id}` :
                                     `/course/${id}/resource/${act.id}`
                                   } 
                                   target="_self" 
                                   style={{color: isDone ? '#2ecc71' : '#555', textDecoration: 'none', fontSize: 13}}
                                >
                                   {act.title}
                                </Link>
                              </div>
      ...
      <div style={{flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0}}>
         ...
         <div style={{background: '#fff', height: 65, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 30px', borderBottom: '1px solid #e0e0e0', position: 'sticky', top: 0, zIndex: 10}}>
            ...
            <div style={{display: 'flex', alignItems: 'center', gap: 20}}>
               {progressPct === 100 && (
                   <Link href={`/course/${id}/certificate`} style={{background: '#f39c12', color: '#fff', textDecoration: 'none', padding: '6px 15px', borderRadius: 20, fontSize: 13, fontWeight: 'bold', boxShadow: '0 2px 10px rgba(243, 156, 18, 0.4)'}}>
                      🏆 Nhận Chứng Chỉ
                   </Link>
               )}
         ...
         <div style={{padding: '35px 50px', flex: 1}}>
            ...
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 30}}>
              ...
              {nextActivity && (
                <Link 
                  href={
                    nextActivity.type === 'quiz' ? `/course/${id}/quiz/${nextActivity.id}` : 
                    nextActivity.type === 'scorm' ? `/course/${id}/scorm/${nextActivity.id}` : 
                    nextActivity.type === 'interactive_video' ? `/course/${id}/interactive-video/${nextActivity.id}` :
                    nextActivity.type === 'game' ? `/course/${id}/game/${nextActivity.id}` : 
                    nextActivity.type === 'assignment' ? `/course/${id}/assignment/${nextActivity.id}` :
                    nextActivity.type === 'forum' ? `/course/${id}/forum/${nextActivity.id}` :
                    `/course/${id}/resource/${nextActivity.id}`
                  }
