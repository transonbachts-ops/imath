      ...
      >
         {activity?.url ? (
            <div style={{
               ...
               <iframe 
                  src={(() => {
                      if (activity.url.includes('studio_player.html')) {
                        const separator = activity.url.includes('?') ? '&' : '?';
                        if (!activity.url.includes('activityId=')) {
                          return `${activity.url}${separator}activityId=${activityId}`;
                        }
                      }
                      return activity.url;
                   })()}
