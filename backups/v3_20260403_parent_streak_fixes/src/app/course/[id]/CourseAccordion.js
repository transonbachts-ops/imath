...
export default function CourseAccordion({ 
  ...
  useEffect(() => {
    // Check for initial PDF preview if URL param exists
    const urlParams = new URLSearchParams(window.location.search);
    const initialPdf = urlParams.get('view_pdf');
  ...
  const renderActivity = (act) => {
    if (act.type === 'label') {
      return (
        <div key={act.id} style={{padding: '15px 20px 15px 45px', borderBottom: '1px solid #f0f0f0', background: '#fff'}}>
          <div dangerouslySetInnerHTML={{ __html: act.details || act.title }} style={{fontSize: 14, color: '#444', lineHeight: 1.6}} />
        </div>
      );
    }

    if (act.type === 'video' && act.url) {
      return (
        <div key={act.id} style={{padding: '20px 20px 20px 45px', borderBottom: '1px solid #eee', background: '#f9f9f9'}}>
           ...
           {act.url.includes('youtube.com') || act.url.includes('youtu.be') ? (
             <div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', borderRadius: 8, boxShadow: '0 4px 12px rgba(0,0,0,0.1)'}}>
               <iframe 
                 ...
                 src={`https://www.youtube.com/embed/${extractYoutubeId(act.url)}`}
                 frameBorder="0"
                 allowFullScreen
                 title="YouTube Video"
               />
             ...
             isParent ? (
               <span style={{color: '#64748b', fontSize: 14, opacity: 0.8}}>Video tư liệu (Chỉ học sinh mới có thể xem)</span>
             ) : (
               <a href={act.url} target="_blank" rel="noreferrer" style={{color: '#1a56db', fontSize: 14, textDecoration: 'underline'}}>Xem Video tại đây</a>
             )
           )}
        </div>
      );
    ...
    return (
      <div key={act.id} style={{padding: '12px 20px 12px 45px', borderBottom: '1px solid #eee', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fff'}}>
         <div style={{display: 'flex', gap: 12, alignItems: 'center', flex: 1}}>
            <span>{getActivityIcon(act.type)}</span>
            <div style={{display: 'flex', flexDirection: 'column'}}>
                {isParent ? (
                  <span style={{color: '#64748b', fontSize: 14, fontWeight: 600, opacity: 0.8}}>{act.title}</span>
                ) : (
                  <Link 
                    href={act.type === 'quiz' ? `/course/${courseId}/quiz/${act.id}` : 
                          act.type === 'scorm' ? `/course/${courseId}/scorm/${act.id}` : 
                          (act.url || '#')} 
                    target={ (act.type === 'quiz' || act.type === 'scorm') ? "_self" : "_blank"} 
                ...
                {isPdf && act.url && act.url.startsWith('/') && !isParent && (
                  <button 
