        ...
        <div className="glass-panel" style={{
          ...
          overflow: 'hidden', zIndex: 999, border: '1px solid rgba(255,255,255,0.4)'
           ...
           <div style={{padding: '8px 0'}}>
              ...
              <Link href="/leaderboard" onClick={() => setOpen(false)} style={{display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', color: '#f39c12', textDecoration: 'none', fontSize: 14, fontWeight: 700}}>
                 <span style={{fontSize: 18}}>🏆</span> Bảng Xếp Hạng Chuyên Cần
              </Link>
              {user.role === 'student' && (
                 <Link href="/daily" onClick={() => setOpen(false)} style={{display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', color: '#e67e22', textDecoration: 'none', fontSize: 14, fontWeight: 700}}>
                   <span style={{fontSize: 18}}>🔥</span> Bài Toán Hằng Ngày
                 </Link>
              )}
              {(user.role === 'admin' || user.role === 'teacher') && (
                 <Link href={user.role === 'teacher' ? '/teacher/dashboard' : '/admin'} onClick={() => setOpen(false)} style={{display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', color: 'var(--primary)', textDecoration: 'none', fontSize: 14, fontWeight: 600}}>
