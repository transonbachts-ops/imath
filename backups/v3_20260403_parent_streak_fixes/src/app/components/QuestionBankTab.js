      ...
      {isEditing ? (
        <form onSubmit={handleSave} style={{background: '#fff9f0', border: '1px solid #f39c12', padding: 25, borderRadius: 12, marginBottom: 30}}>
          ...
          <div style={{marginBottom: 15}}>
            <label style={{display: 'block', fontWeight: 'bold', marginBottom: 5}}>Nội dung câu hỏi</label>
            <textarea required value={form.question_text || ''} onChange={e => setForm({...form, question_text: e.target.value})} rows={3} style={{width: '100%', padding: 10, borderRadius: 6, border: '1px solid #ccc'}} />
          </div>
          ...
          <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15, marginBottom: 20}}>
             {['A', 'B', 'C', 'D'].map(k => (
               <div key={k} style={{display: 'flex', alignItems: 'center', gap: 10}}>
                 ...
                 <input value={form.options_json[k] || ''} onChange={e => setForm({...form, options_json: {...form.options_json, [k]: e.target.value}})} style={{flex: 1, padding: 8, borderRadius: 4, border: '1px solid #ccc'}} />
               </div>
             ))}
          </div>
        ...
        <div style={{display: 'flex', flexDirection: 'column', gap: 15}}>
          ...
          {questions.map(q => {
            ...
            <div key={q.id} style={{padding: 20, border: '1px solid #eee', borderRadius: 12, background: '#fafafa', position: 'relative'}}>
              ...
              <div style={{fontWeight: 'bold', color: '#003380', marginBottom: 10, fontSize: 16, paddingRight: 60}}>{q.question_text}</div>
              <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5, fontSize: 13, marginBottom: 10}}>
                 {['A', 'B', 'C', 'D'].map(k => (
                   <div key={k} style={{color: q.correct_answer === k ? '#2ecc71' : '#555', fontWeight: q.correct_answer === k ? 'bold' : 'normal'}}>{k}. {opts[k]} {q.correct_answer === k && '✅'}</div>
                 ))}
              </div>
              <div style={{display: 'flex', gap: 10}}>
