...
export default function CalendarBoard({ variant = 'default', dailyQuizzes = [] }) {
    ...
    <div style={{width: '100%', marginBottom: variant === 'full' ? 0 : 40}}>
      ...
      <div className="glass-panel" style={{
        ...
        {Array.from({length: daysInMonth}).map((_, i) => {
          ...
          const isPastOrToday = cellDate < today || isToday;
          let dailyQuizUi = null;
          
          if (isPastOrToday && window.location.pathname.includes('/dashboard')) {
             const dateKey = `${year}-${String(month+1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
             const dq = dailyQuizzes.find(d => d.date === dateKey);
             if (dq) {
                dailyQuizUi = <div style={{marginTop: 6, fontSize: 11, fontWeight: 800, color: '#15803d'}}><span style={{background: '#dcfce7', padding: '2px 6px', borderRadius: 4, display: 'inline-block'}}>v {dq.score}đ</span></div>;
             } else {
                dailyQuizUi = <div style={{marginTop: 6, fontSize: 11, fontWeight: 900, color: '#b91c1c'}}><span style={{background: '#fee2e2', padding: '2px 8px', borderRadius: 4, display: 'inline-block'}}>x</span></div>;
             }
          }
          ...
          return (
             <div key={day} style={{
               ...
               <div style={{display: 'flex', flexDirection: 'column', gap: 6}}>
                 {dayEvents.map(e => (
                   e.type === 'zoom' ? 
                   <a key={e.id} href={e.zoom_link} target="_blank" rel="noreferrer" title={`Zoom: ${e.title}`} style={{
