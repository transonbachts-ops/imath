...
export default async function MyCoursesPage() {
  ...
  return (
    <div style={{fontFamily: 'system-ui, sans-serif', background: '#fff', minHeight: '100vh', paddingBottom: 80}}>
      {/* HEADER */}
      ...
      <div style={{maxWidth: 1200, margin: '40px auto 0', display: 'flex', gap: 40}}>
         ...
         <div style={{width: 250, flexShrink: 0, borderRight: '1px solid #eee', paddingRight: 20}}>
            <ul style={{listStyle: 'none', padding: 0, margin: 0}}>
               <li style={{marginBottom: 10}}><Link href="/profile" style={{display: 'block', padding: '12px 15px', color: '#555', textDecoration: 'none', borderRadius: 8, fontSize: 15, transition: '0.2s'}}>👤 Hồ sơ cá nhân</Link></li>
               <li style={{marginBottom: 10}}><Link href="/my-courses" style={{display: 'block', padding: '12px 15px', color: '#003380', background: '#f0f4ff', textDecoration: 'none', borderRadius: 8, fontSize: 15, fontWeight: 'bold'}}>📚 Khóa học của tôi</Link></li>
            </ul>
         </div>
         ...
         <div style={{flex: 1}}>
           {courses.length === 0 ? (
             <div style={{textAlign: 'center', padding: '100px 0', background: '#fff', borderRadius: 12, border: '1px solid #eee'}}>
               <p style={{fontSize: 18, color: '#666', marginBottom: 20}}>Bạn chưa tham gia vào Khóa học nào.</p>
             ...
             <div style={{display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 30}}>
                 {courses.map(course => {
                    ...
                    return (
                       <div key={course.id} style={{background: '#fff', borderRadius: 12, overflow: 'hidden', border: '1px solid #e0e0e0', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', display: 'flex', flexDirection: 'column'}}>
                          <div style={{position: 'relative', height: 160, backgroundImage: `url(${course.image_url})`, backgroundSize: 'cover', backgroundPosition: 'center'}}>
                             ...
                             {/* Generic Logo Mock */}
                             <div style={{position: 'absolute', bottom: 10, left: 10, background: '#fff', padding: 5, borderRadius: 6, display: 'inline-block'}}>
                                <span style={{fontWeight: 900, color: '#333'}}>H2bmath<span style={{color: '#e74c3c'}}>.</span></span>
                             </div>
                          </div>
                          <div style={{padding: '20px', flexGrow: 1, display: 'flex', flexDirection: 'column'}}>
                             <h3 style={{fontSize: 16, color: '#333', fontWeight: 700, marginBottom: 20, lineHeight: 1.4, flexGrow: 1}}>{course.title}</h3>
                             ...
                             <p style={{color: '#666', fontSize: 13, marginBottom: 20}}>Tiến độ: <strong>{prog.pct}%</strong> ({prog.done}/{prog.total} bài học)</p>
                             
                             {/* Link to learning dashboard */}
                             <Link href={`/course/${course.id}/learn`} style={{background: 'transparent', color: '#333', border: '1px solid #ccc', textAlign: 'center', padding: '10px 0', borderRadius: 6, fontWeight: 'bold', fontSize: 13, textDecoration: 'none', display: 'block', transition: '0.2s'}}>
                               MỞ BẢNG ĐIỀU KHIỂN
                             </Link>
                          </div>
                       </div>
