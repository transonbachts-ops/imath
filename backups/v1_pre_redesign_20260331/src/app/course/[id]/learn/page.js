                         ...
                         {acts.map(act => {
                           ...
                           return (
                             <div key={act.id} style={{padding: '8px 15px 8px 0', fontSize: 13, display: 'flex', gap: 8, alignItems: 'center', borderBottom: '1px solid #f9f9f9'}}>
                               ...
                               <Link href={act.type === 'quiz' ? `/course/${id}/quiz/${act.id}` : (act.url && !/ex[ae]mple/i.test(act.url) ? act.url : '#')} target={act.type === 'quiz' || (act.url && /ex[ae]mple/i.test(act.url)) ? '_self' : '_blank'} style={{color: isDone ? '#2ecc71' : '#555', textDecoration: 'none', fontSize: 13}}>
                                  {act.title}
                               </Link>
                             </div>
                           );
      ...
      <div style={{flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0}}>
         ...
         <div style={{padding: '35px 50px', flex: 1}}>
            ...
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 30}}>
              ...
              {nextActivity && (
                <Link 
                  href={nextActivity.type === 'quiz' ? `/course/${id}/quiz/${nextActivity.id}` : (nextActivity.url && !/ex[ae]mple/i.test(nextActivity.url) ? nextActivity.url : '#')}
                  target={nextActivity.type === 'quiz' || (nextActivity.url && /ex[ae]mple/i.test(nextActivity.url)) ? '_self' : '_blank'}
