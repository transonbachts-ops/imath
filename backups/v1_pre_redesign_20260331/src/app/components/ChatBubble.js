...
export default function ChatBubble() {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div style={{
          position: 'fixed', bottom: 100, right: 30, width: 350, height: 500, background: '#fff',
          borderRadius: 20, boxShadow: '0 10px 40px rgba(0,0,0,0.15)', zIndex: 9998,
          display: 'flex', flexDirection: 'column', border: '1px solid #eee', overflow: 'hidden'
        }}>
          {/* HEADER */}
          <div style={{
            ...
            display: 'flex', alignItems: 'center', gap: 10
             ...
             <div style={{color: '#ffffff'}}>
               <div style={{fontWeight: 900, fontSize: 18, textShadow: '0 1px 2px rgba(0,0,0,0.2)'}}>H2bmath AI</div>
               <div style={{fontSize: 13, fontWeight: 500, opacity: 0.95}}>Trợ lý học tập thông minh</div>
             </div>
          </div>

          {/* MESSAGES */}
          <div ref={scrollRef} style={{flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 15, background: '#f8fafa'}}>
            {messages.map((m, i) => (
              <div key={i} style={{
                ...
                {m.role === 'model' ? (
                  <div className="markdown-body" style={{fontSize: 14}} dangerouslySetInnerHTML={{__html: 
                    m.text
            ...
            {loading && (
              <div style={{alignSelf: 'flex-start', background: '#fff', padding: '10px 15px', borderRadius: '18px 18px 18px 4px', border: '1px solid #e0e0e0', fontSize: 13, color: '#888'}}>
                Đang suy nghĩ...
              </div>
            )}
          </div>
          ...
          {/* INPUT AREA */}
          <div style={{padding: '12px 15px', background: '#fff', borderTop: '1px solid #eee', display: 'flex', alignItems: 'center', gap: 10}}>
            <input 
              type="text" 
              ...
              style={{flex: 1, padding: '10px 15px', borderRadius: 20, border: '1px solid #ddd', outline: 'none', background: '#f5f5f5', fontSize: 14}}
            />
            <button 
              onClick={sendMessage}
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .markdown-body strong { font-weight: bold; color: #111; }
        ...
        .markdown-body th { background: #f0f4f8; }
      `}} />
    </>
  );
}
