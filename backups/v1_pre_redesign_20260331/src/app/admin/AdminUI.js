...
export default function AdminUI({ userRole = 'admin', userId }) {
  ...
  const [teacherForm, setTeacherForm] = useState({ name: '', role_title: '', bio: '', avatar_url: '' });
  ...
  const handleSaveTeacher = async (e) => {
    e.preventDefault();
    await fetch('/api/admin/teachers', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(teacherForm) });
    setEditingTeacher(null);
    setTeacherForm({ name: '', role_title: '', bio: '', avatar_url: '' });
    fetchTeachers();
  };
              ...
              {courses.map(c => (
                 <div key={c.id} style={{background: '#fff', padding: 25, borderRadius: 20, boxShadow: '0 10px 20px rgba(0,0,0,0.03)'}}>
                   ...
                   <div style={{display: 'flex', gap: 10, marginTop: 'auto'}}>
                     <Link href={`/admin/course/edit/${c.id}`} style={{flex: 1, padding: 8, background: '#f5f6fa', border: '1px solid #ddd', borderRadius: 6, cursor: 'pointer', textAlign: 'center', textDecoration: 'none', color: '#333'}}>✒️ Chỉnh Sửa</Link>
                     <button style={{background: '#e74c3c', color: '#fff', padding: 8, border: 'none', borderRadius: 6, cursor: 'pointer'}} onClick={() => handleDeleteCourse(c.id)}>Xóa</button>
                   </div>
                 </div>
              ))}
            </div>
        ...
        {tab === 'teachers' && (
          <div style={{background: '#fff', borderRadius: 16, padding: 30, boxShadow: '0 5px 20px rgba(0,0,0,0.04)'}}>
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 25}}>
              ...
              <button 
                onClick={() => { setEditingTeacher(true); setTeacherForm({name:'',role_title:'',bio:'', avatar_url: ''}); }} 
              ...
              <form onSubmit={handleSaveTeacher} style={{background:'#f0faf5',border:'1px solid #b2dfdb',borderRadius:12,padding:25,marginBottom:30}}>
                ...
                <div style={{display:'grid', gridTemplateColumns:'120px 1fr', gap:25, marginBottom:20}}>
                   <div style={{textAlign: 'center'}}>
                      <div style={{width: 100, height: 100, borderRadius: '50%', background: '#fff', border: '1px solid #ddd', marginBottom: 10, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                         {teacherForm.avatar_url && teacherForm.avatar_url.startsWith('http') ? (
                           <img src={teacherForm.avatar_url} style={{width:'100%', height:'100%', objectFit: 'cover'}} alt="Avatar" />
                   ...
                   <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:15}}>
                      ...
                      <div style={{gridColumn: 'span 2'}}>
                        ...
                        <textarea value={teacherForm.bio} onChange={e=>setTeacherForm({...teacherForm,bio:e.target.value})} placeholder="Giới thiệu kinh nghiệm..." rows={3} style={{width:'100%',padding:'10px',borderRadius:6,border:'1px solid #ccc', resize: 'vertical'}}/>
                      </div>
                   </div>
                </div>
