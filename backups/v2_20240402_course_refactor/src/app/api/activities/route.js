...
export async function POST(req) {
  ...
  const { module_id, title, type, url, order_index } = await req.json();
  if (!module_id || !title || !type) return NextResponse.json({ error: 'Missing required fields' }, { status: 400 });

  const [res] = await pool.query(
    'INSERT INTO course_activities (module_id, title, type, url, order_index) VALUES (?, ?, ?, ?, ?)',
    [module_id, title, type, url || '', order_index || 0]
  );
  
  return NextResponse.json({ id: res.insertId, module_id, title, type, url, order_index });
}
