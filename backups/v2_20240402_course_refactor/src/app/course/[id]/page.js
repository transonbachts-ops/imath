      ...
      <div style={{maxWidth: 1000, margin: '0 auto', marginTop: 40, background: '#fff', padding: 60, borderRadius: 8, boxShadow: '0 5px 25px rgba(0,0,0,0.03)'}}>
            ...
            <div style={{flex: '1'}}>
               ...
               {teacher && (
                 <div style={{marginTop: 20, display: 'flex', alignItems: 'center', gap: 12, background: '#f0f7ff', padding: '12px 16px', borderRadius: 8, border: '1px solid #c5e0ff'}}>
                   <div style={{width: 44, height: 44, borderRadius: '50%', background: '#003380', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 18, flexShrink: 0}}>
                     {teacher.avatar_url ? (
                       <img src={teacher.avatar_url} style={{width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover'}} alt="Teacher" />
                     ) : (
                       teacher.name.split(' ').pop()[0]
                     )}
                   </div>
                   <div>
                     <div style={{fontSize: 12, color: '#666', marginBottom: 2}}>Giảng viên phụ trách</div>
                     <div style={{fontWeight: 700, color: '#003380', fontSize: 15}}>{teacher.name}</div>
         ...
         <div style={{display: 'flex', gap: 15, marginBottom: 40, borderBottom: '1px solid #eee', paddingBottom: 40, flexWrap: 'wrap'}}>
            ...
            {course.textbook_url && enrollmentStatus === 'approved' && (
               <a href={course.textbook_url} target="_blank" rel="noreferrer" style={{background: '#cc0000', color: '#fff', padding: '12px 25px', fontSize: 15, fontWeight: 'bold', textDecoration: 'none', borderRadius: 4}}>Đọc SGK Môn Học</a>
            )}
            {course.lesson_plan_url && (
               <a href={course.lesson_plan_url} target="_blank" rel="noreferrer" style={{background: '#003380', color: '#fff', padding: '12px 25px', fontSize: 15, fontWeight: 'bold', textDecoration: 'none', borderRadius: 4}}>Tải Giáo Án Đính Kèm</a>
            )}
         </div>

         {/* ACCORDION KHÓA HỌC */}
         {enrollmentStatus === 'approved' || user?.role === 'admin' || user?.role === 'teacher' ? (
           <CourseAccordion 
